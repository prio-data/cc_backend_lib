...
class SchedulerClient(api_client.ApiClient):
    def deserialize(self, data: bytes) -> Either[http_error.HttpError, schema.time_partition.TimePartition]:
        try:
            return Right(schema.time_partition.TimePartition(**json.loads(data)))
        except Exception as e:
            return Left(http_error.HttpError(message = str(e), http_code = 500))

    async def time_partition(self, shift: int = 0) -> Either[http_error.HttpError, schema.time_partition.TimePartition]:
